...
import { WebGPURenderer } from './renderers/webgpu-renderer.ts'
...
import type { JASSUB, MainModule } from '../wasm/types'
...
export class ASSRenderer {
  ...
  constructor (data: opts, getFont: (font: string, weight: WeightValue) => Promise<Uint8Array<ArrayBuffer> | undefined>) {
    ...
    this._ready = (WASM({ __url: data.wasmUrl, __out: (log: string) => this._log(log) }) as Promise<MainModule>).then(async ({ _malloc, JASSUB }) => {
      ...
      this._gpurender = new WebGPURenderer((await devicePromise)!)
      if (this._offCanvas) this._gpurender.setCanvas(this._offCanvas)
  ...
  _resizeCanvas (width: number, height: number) {
    ...
    this._wasm.resizeCanvas(width, height, width, height)
